...
# load ne_110m from file
world = gpd.read_file('geopandas_maps/ne_110m_admin_0_countries.shx')

# User added cities get converted to coordinates with nominatim
cities = []
list_prompt = ""
...
def euc_dist(c1, c2): #euclidean distance for measurements
...
def tot_dist(rt, dist_matrix):
    ...
    return distance #distance of route

def neighbor(rt): #neighbor lengths
    neighbor_rt = rt[:]
...
# convert lat/lon coords to cartesian coords
...
#print(world)
world.plot(ax=ax, column='CONTINENT', cmap='Pastel1', legend=False, edgecolor='black')
route_gdf.plot(ax=ax, color='blue', linewidth=2)
plt.scatter([lon for lat, lon in cities], [lat for lat, lon in cities], color='red', zorder=5)
plt.show()